...
@editor: Riccardo Pazzi: added support for multiple URMs in phase of evaluation
"""
...
def _create_empty_metrics_dict(cutoff_list, n_items, n_users, URM_train, URM_test, ignore_items, ignore_users,
...
def get_result_string_df(results_run_df, n_decimals=7):
    ...
    for cutoff in results_run_df.index:
        ...
        for metric in results_run_df.columns:
            output_str += "{}: {:.{n_decimals}f}, ".format(metric, results_run_df.loc[cutoff, metric],
...
class Evaluator(object):
    ...
    def __init__(self, URM_test_list, cutoff_list, min_ratings_per_user=1, exclude_seen=True,
        ...
        if not np.all(users_to_evaluate_mask):
            self._print("Ignoring {} ({:4.1f}%) Users that have less than {} test interactions".format(
                np.sum(users_to_evaluate_mask),
    ...
    def _compute_metrics_on_recommendation_list(self, test_user_batch_array, recommended_items_batch_list, scores_batch,
                                                results_dict):
        ...
        for batch_user_index in range(len(recommended_items_batch_list)):
            ...
            for cutoff in self.cutoff_list:
                ...
                results_current_cutoff[EvaluatorMetrics.COVERAGE_ITEM.value].add_recommendations(
                    recommended_items_current_cutoff)
                results_current_cutoff[EvaluatorMetrics.COVERAGE_ITEM_CORRECT.value].add_recommendations(
                ...
                results_current_cutoff[EvaluatorMetrics.COVERAGE_USER_CORRECT.value].add_recommendations(
...
class EvaluatorMultipleURMs(object):
    """EvaluatorMultipleURMs adds support for lists of URMs instead of a single URM"""

    EVALUATOR_NAME = "EvaluatorMultipleURMs"
    ...
    def __init__(self, URM_test_list, cutoff_list, min_ratings_per_user=1, exclude_seen=True,
                 ...
                 ignore_users=None,
                 verbose=True):
        # Create a list of EvaluatorHoldout objects on URMs in the list
        # Initialize parameters
        self.URM_test_list = URM_test_list
        self.Evaluator_list = []
        self.results = []
        self.map_list = []
        for URM_test in URM_test_list:
            dummy_evaluator = EvaluatorHoldout(URM_test, cutoff_list=cutoff_list,
                                               min_ratings_per_user=min_ratings_per_user, exclude_seen=exclude_seen,
                                               diversity_object=diversity_object, ignore_items=ignore_items,
    ...
    def print_map_statistics(self):
        array_np = np.array(self.map_list)
        print("Average MAP: {}, Minimum: {}, Maximum: {} \n Variance: {}, Median: {}".format(np.mean(array_np),
                                                                                             np.min(array_np),
                                                                                             np.max(array_np),
                                                                                             np.var(array_np),
                                                                                             np.median(array_np)
