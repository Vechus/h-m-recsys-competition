...
if __name__ == '__main__':
    ...
    save_path = os.path.join(path, "{}-submission-final.csv".format("toppop_weight_decayed"))

    f = open(save_path, "w")
    f.write("customer_id,prediction\n")

    # for i in df_sample_submission.customer_id.unique():
    #     recommended_items = recommender_pop_weight.recommend(i)
    #     f.write(f"{i}, {recommended_items}\n")
    ...
    recommender_pop_weight.recommend(df_sample_submission.customer_id.unique(), f)
    f.close()
    print("save complete")
